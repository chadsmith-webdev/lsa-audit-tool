...
import TechnicalMonitor from "./TechnicalMonitor";

export const metadata: Metadata = {
  title: "Technical Monitor — Local Search Ally",
  description:
    "On-demand technical SEO scan with measured response time, schema, indexability, and AI fix guidance.",
};

export default async function TechnicalToolPage() {
  ...
  await requireProAccess(user.id, "technical");
      ...
      <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
        ...
        >
          ...
          <p className='text-small' style={{ marginBottom: "var(--space-6)" }}>
            Run an audit first so we have your website on file.
          </p>
  ...
  const inputObj =
    latest.input && typeof latest.input === "object"
      ? (latest.input as { websiteUrl?: string })
      : null;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      ...
      >
        ...
        <TechnicalMonitor
          auditId={latest.id}
          defaultUrl={inputObj?.websiteUrl ?? null}
        />
      </main>
    </div>
  );
